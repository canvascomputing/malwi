...
pub(crate) fn ensure_default_policy(path: &Path) -> Result<()> {
    ...
    std::fs::write(
        path,
        malwi_policy::templates::DEFAULT_SECURITY_YAML.trim_start(),
    )?;
    Ok(())
}
...
pub(crate) fn reset_policies() -> Result<()> {
    ...
    for name in &names {
        let yaml = if *name == "default" {
            malwi_policy::templates::DEFAULT_SECURITY_YAML
                .trim_start()
                .to_string()
        } else {
...
pub(crate) fn write_policy(name: &str) -> Result<()> {
    let yaml = if name == "default" {
        ...
        malwi_policy::templates::embedded_policy(name).ok_or_else(|| {
            anyhow::anyhow!(
