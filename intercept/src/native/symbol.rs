...
pub fn enumerate_modules() -> Vec<ModuleInfo> {
    crate::module::enumerate_modules()
        .into_iter()
        .map(|m| ModuleInfo {
            name: m.name,
            path: m.path,
...
pub fn enumerate_exports(module_name: &str) -> Vec<ExportInfo> {
    match crate::module::enumerate_exports(module_name) {
        Ok(exports) => exports
            .into_iter()
            .map(|e| ExportInfo {
                name: e.name,
...
/// Uses crate::module::enumerate_symbols to find both exported and local symbols.
pub fn find_symbol(module_name: &str, symbol: &str) -> Result<usize> {
    let symbols = crate::module::enumerate_symbols(module_name)
...
pub fn find_export(module: Option<&str>, symbol: &str) -> Result<usize> {
    let address = if let Some(mod_name) = module {
        crate::module::find_export_by_name(mod_name, symbol)
    } else {
        crate::module::find_global_export_by_name(symbol)
    };
...
pub fn find_symbols_matching(module: Option<&str>, pattern: &str) -> Vec<ExportInfo> {
    ...
    if !is_glob_pattern(pattern) {
        ...
        for m in modules {
            if let Ok(symbols) = crate::module::enumerate_symbols(&m.name) {
                for s in symbols {
    ...
    for module_info in modules {
        let symbols = match crate::module::enumerate_symbols(&module_info.name) {
            Ok(s) => s,
            Err(_) => continue,
        };
