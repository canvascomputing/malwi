...
# Legacy hardcoded URLs (commented out - now loaded from JSON)
...
def get_or_clone_repo(repo_url, target_cache_subdir):
    ...
    # Get pinned commit if available (from pinned_repositories.json)
    pinned_commit = get_pinned_commit_for_url(repo_url)

    # Use embedded commit from URL if no pinned commit found
    if not pinned_commit and url_commit:
        pinned_commit = url_commit
